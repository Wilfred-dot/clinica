...
const statusBadgeClasses = {
  activo:   { bg: 'bg-[#edf7f2]', text: 'text-[#10b981]', dot: 'bg-[#10b981]' },
    ...
    <Shell>
      ...
      <div className="flex items-start justify-between gap-4 mb-7 flex-wrap">
        <div>
          ...
          <p className="text-[13px] text-[#6b8299] mt-0.5 font-medium">Base de dados de pacientes registados na clínica</p>
        </div>
        <Link
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-[#ecf1f6] bg-[#f8fafc]">
                  ...
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">Contacto</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">Último Atendimento</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">Estado</th>
                  <th className="p-3.5 pr-5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299] text-right">Ações</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#ecf1f6]">
                {filtered.map(patient => {
                  ...
                  return (
                    <tr key={patient.id} className="hover:bg-[#fdfeff] transition-colors">
                      ...
                      <td className="p-4 text-sm font-medium text-[#6b8299]">
                      ...
                      <td className="p-4 pr-5 text-sm text-right">
                        <div className="flex gap-2 justify-end">
                          <Link
                            ...
                            className="inline-flex items-center rounded-[6px] px-2.5 py-1.5 text-xs font-bold border border-[#d6e0ea] bg-white text-[#102A6B] transition hover:bg-[#f1f5f9]"
                          >
                            Histórico
                          </Link>
                          <Link
                            href={`/admin/patients/${patient.id}/editar`}
                            className="inline-flex items-center rounded-[6px] border border-[#d6e0ea] bg-white px-2.5 py-1.5 text-xs font-bold text-[#6b8299] transition hover:bg-[#f1f5f9]"
                          >
                            Editar
                          </Link>
                          {ativo ? (
                            ...
                            <button
                              ...
                              disabled={actionLoading === patient.user_id}
                              className="inline-flex items-center rounded-[6px] px-2.5 py-1.5 text-xs font-bold bg-[#edf7f2] text-[#10b981] border border-[#a7f3d0] transition hover:bg-[#d1fae5] disabled:opacity-50"
                            >
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={5} className="text-center text-[#6b8299] font-medium p-12 text-sm">

...
export default function PatientDetailPage() {
  ...
  const statusColors: Record<string, string> = {
    realizada:  'bg-[#edf7f2] text-[#10b981]',
    ...
    <Shell>
      ...
      <div className="flex items-start justify-between gap-4 mb-7 flex-wrap">
        <div>
          ...
          <p className="text-[13px] text-[#6b8299] mt-0.5 font-medium">Histórico clínico detalhado do utente</p>
        </div>
        <div className="flex gap-2">
          ...
          <Link
            href="/admin/patients"
            className="inline-flex items-center gap-1.5 rounded-[8px] border border-[#d6e0ea] bg-white px-4 h-10 text-[13.5px] font-bold text-[#6b8299] transition hover:bg-[#f1f5f9]"
          >
            ← Voltar
          </Link>
        </div>
      ...
      <div className="bg-white border border-[#ecf1f6] rounded-[12px] shadow-[0_2px_4px_rgba(16,42,107,.03)] overflow-hidden">
        ...
        {loading ? (
          <p className="p-12 text-center text-sm font-medium text-[#6b8299] animate-pulse">A carregar registos...</p>
        ) : (
              ...
              <thead>
                <tr className="border-b border-[#ecf1f6] bg-[#f8fafc]">
                  {['Data', 'Médico', 'Status', 'Motivo', 'Diagnóstico', 'Prescrições'].map(h => (
                    <th key={h} className="p-3.5 pl-5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {consultas.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-[#fdfeff] transition-colors">
                      <td className="p-4 pl-5 text-sm font-medium text-[#6b8299] whitespace-nowrap">
                      ...
                      <td className="p-4 text-sm">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11.5px] font-bold capitalize ${statusColors[statusNormalizado] || 'bg-[#f1f5f9] text-[#6b8299]'}`}>
                          {statusNormalizado}
                        </span>
                      </td>

    ...
    <Shell>
      <div className="flex items-start justify-between gap-4 mb-7 flex-wrap">
        <div>
          ...
          <p className="text-[13px] text-[#6b8299] mt-0.5 font-medium">Corpo clínico da Clínica MMQ Oftalmologia</p>
        </div>
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-[#ecf1f6] bg-[#f8fafc]">
                  <th className="p-3.5 pl-5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">Nome</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">Especialidade</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">N.º Registo</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">Consultas (Mês)</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299]">Estado</th>
                  <th className="p-3.5 pr-5 text-[11px] font-bold uppercase tracking-[0.6px] text-[#6b8299] w-44">Ações</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#ecf1f6]">
                {filtered.map(medico => (
                  <tr key={medico.id} className="hover:bg-[#fdfeff] transition-colors">
                    ...
                    <td className="p-4 text-sm font-medium text-[#6b8299]">{medico.numero_ordem}</td>
                    <td className="p-4 text-sm font-semibold text-[#102A6B]">{medico.consultasMes}</td>
                    <td className="p-4 text-sm">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[12px] font-semibold ${medico.users?.ativo ? 'bg-[#edf7f2] text-[#10b981]' : 'bg-[#fdf0f0] text-[#ef4444]'}`}>
                    ...
                    <td className="p-4 pr-5 text-sm">
                      <div className="flex gap-2">
                        <Link href={`/admin/medics/${medico.id}/editar`} className="inline-flex items-center rounded-[6px] border border-[#d6e0ea] bg-white px-3 py-1.5 text-[12.5px] font-bold text-[#6b8299] transition hover:bg-[#f1f5f9]">
                          Editar
                        </Link>
                        <button
                          className={`inline-flex items-center rounded-[6px] px-3 py-1.5 text-[12.5px] font-bold border transition disabled:opacity-50 ${
                            medico.users?.ativo 
                              ? 'bg-[#fffbeb] border-[#fde8bb] text-[#b87a00] hover:bg-[#fef3c7]' 
                              : 'bg-[#edf7f2] border-[#a8ddc0] text-[#10b981] hover:bg-[#d1fae5]'
                          }`}
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center p-12 text-sm font-medium text-[#6b8299]">

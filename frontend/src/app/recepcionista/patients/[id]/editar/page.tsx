...
export default function ReceptionEditPatientPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (patient?.user_id) {
        ...
        await request(`/users/${patient.user_id}`, { method: 'PATCH', body: JSON.stringify(userBody) });
      }
      router.push('/recepcionista/patients');
    } catch (err: any) {
      setError(err.message || 'Erro ao actualizar paciente');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Shell>
      <div className="ph">
        <div><h1>Editar Paciente</h1><p className="sub">{patient.users?.name} · {patient.users?.email || 'Sem email'}</p></div>
      ...
      <form onSubmit={handleSubmit} className="form-panel">
        ...
        <div className="form-row">
          <div className="field"><label>Nova senha <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0, color: 'var(--ink4)' }}>(deixar em branco para manter)</span></label><input type="password" value={password} onChange={e => setPassword(e.target.value)} /></div>
          <div className="field"><label>Estado</label><select value={ativo ? 'true' : 'false'} onChange={e => setAtivo(e.target.value === 'true')}><option value="true">Activo</option><option value="false">Inactivo</option></select></div>
        </div>
        <div className="form-actions">
          <button type="submit" className="btn btn-primary" disabled={loading}>{loading ? 'Guardando...' : 'Guardar alterações'}</button>
        ...
        <button type="button" className="btn btn-sm" style={{ background: 'var(--danger-dim)', color: 'var(--danger)', border: '1px solid #f0c4c4' }} onClick={() => setShowDelete(true)}>Eliminar paciente</button>
        <ConfirmModal open={showDelete} title="Eliminar paciente" message={`Tem a certeza de que pretende eliminar ${patient.users?.name}?`} confirmLabel="Eliminar" cancelLabel="Cancelar" onConfirm={handleDelete} onCancel={() => setShowDelete(false)} variant="danger" />
      </form>
    </Shell>
  );
}

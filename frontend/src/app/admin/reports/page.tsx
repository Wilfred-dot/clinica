...
// Função para extrair o nome do paciente de diferentes formatos de resposta
function getPacienteNome(notificacao: Notificacao): string {
  if (notificacao.paciente) {
    return notificacao.paciente;
  }
  if (notificacao.pacientes?.users?.name) {
    return notificacao.pacientes.users.name;
  }
  return '';
}
            ...
            <table className="w-full border-collapse min-w-[600px]">
              <thead>
                <tr className="bg-[var(--slate)] border-b border-[var(--border2)]">
                  <th className="p-[10px_18px] text-left text-[11px] font-bold text-[var(--ink3)] uppercase tracking-[0.7px]">Data</th>
                  <th className="p-[10px_18px] text-left text-[11px] font-bold text-[var(--ink3)] uppercase tracking-[0.7px]">Paciente</th>
                  <th className="p-[10px_18px] text-left text-[11px] font-bold text-[var(--ink3)] uppercase tracking-[0.7px]">Tipo</th>
                  <th className="p-[10px_18px] text-left text-[11px] font-bold text-[var(--ink3)] uppercase tracking-[0.7px]">Mensagem</th>
                </tr>
              </thead>
              <tbody>
                {notificacoes.map(n => {
                  const badge = tipoBadge[n.tipo_variavel] || { bg: 'bg-[var(--slate2)]', text: 'text-[var(--ink3)]', dot: 'bg-[var(--ink4)]', label: n.tipo_variavel };
                  const nomePaciente = getPacienteNome(n);
                  return (
                    <tr key={n.id} className="border-b border-[var(--border2)] last:border-b-0 hover:bg-[var(--slate)] transition">
                      ...
                      <td className="p-[12px_18px] text-[13.5px] text-[var(--ink)] font-semibold">
                        {nomePaciente ? (
                          ...
                          <span className="text-[var(--ink4)] italic">—</span>
                        )}
                      </td>

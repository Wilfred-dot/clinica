  ...
  return (
    <Shell>
      ...
      <div className="flex items-start justify-between gap-4 mb-7 flex-wrap">
        <div>
          <h1 className="text-[24px] font-bold text-[var(--ink)] tracking-[-0.5px]">Lista de Pacientes</h1>
          <p className="text-[13px] text-ink-3 mt-0.5 font-medium">Todos os pacientes registados na clínica</p>
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-[var(--border2)] bg-[var(--slate)] sticky top-0 z-10">
                  <th className="p-3.5 pl-5 text-[11px] font-bold uppercase tracking-[0.6px] text-ink-3 w-[25%]">Nome</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-ink-3 w-[10%] whitespace-nowrap">Idade</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-ink-3 w-[15%]">Género</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-ink-3 w-[20%]">Contacto</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-ink-3 w-[8%] whitespace-nowrap">Consultas</th>
                  <th className="p-3.5 text-[11px] font-bold uppercase tracking-[0.6px] text-ink-3 w-[10%]">Estado</th>
                  <th className="p-3.5 pr-5 text-[11px] font-bold uppercase tracking-[0.6px] text-ink-3 text-right w-[12%]">Ações</th>
                </tr>
              </thead>
                ...
                {displayedPatients.map((patient, index) => {
                  ...
                  return (
                    <tr 
                      ...
                      onClick={() => router.push(`/admin/patients/${patient.id}/editar`)}
                      style={{
                      ...
                      <td className="p-4 pr-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Link
                            ...
                            className="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-md border border-[var(--border)] text-[var(--ink)] hover:bg-[var(--slate)] transition-colors min-w-[70px] justify-center whitespace-nowrap"
                            onClick={(e) => e.stopPropagation()}
                          >
                            Editar
                          </Link>
                          {ativo ? (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                setConfirm({ patientId: patient.id, userId: patient.user_id, action: 'deactivate' });
                              }}
                              disabled={actionLoading === patient.user_id}
                              className="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-md border border-red-300 dark:border-red-700 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors min-w-[80px] justify-center whitespace-nowrap"
                            >
                              {actionLoading === patient.user_id ? '...' : 'Desactivar'}
                            </button>
                          ) : (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                setConfirm({ patientId: patient.id, userId: patient.user_id, action: 'activate' });
                              }}
                              disabled={actionLoading === patient.user_id}
                              className="inline-flex items-center px-3 py-1.5 text-xs font-bold rounded-md bg-[var(--mmq-orange)] text-white hover:bg-[var(--mmq-orange-lt)] transition-colors min-w-[80px] justify-center whitespace-nowrap"
                            >
          ...
          <div className="bg-[var(--white)] rounded-xl shadow-xl p-6 max-w-md w-full mx-4 border border-[var(--border2)]">
            <div className="flex flex-col items-center mb-4">
              ...
              <h3 className="text-lg font-bold text-[var(--ink)] mt-3">
                {confirm.action === 'deactivate' ? 'Desactivar paciente' : 'Activar paciente'}
              </h3>
            </div>
